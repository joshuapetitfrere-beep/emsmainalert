...
// Transport status options — order matters (left to right)
...
export default function EMSScreen({ onExit }: Props) {
  ...
  function sendStatus(status: TransportStatus) {
    ...
    // "Arrived" resets the alert log ACK count and clears the active alert ID
    if (status === "arrived") {
      setLastAlertId(null);
      setAckCount(0);
    }
...
const styles = StyleSheet.create({
  ...
  transportBar:         { backgroundColor: "#111827", borderRadius: 14, padding: 16, borderWidth: 1 },
