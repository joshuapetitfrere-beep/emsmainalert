...
export default function EMSScreen({ onExit, onOpenSettings }: Props) {
  // Credentials loaded from AsyncStorage
  ...
  useEffect(() => {
    Promise.all([
      ...
      if (id) setEmsUnitId(id);
      if (key) setEmsApiKey(key);
      setCredentialsLoaded(true);
    });
  }, []);

  // ── Start app once credentials are loaded ──────────────────────────────────
  useEffect(() => {
    if (!credentialsLoaded || !emsUnitId || !emsApiKey) return;
    startLocation();
    connectWebSocket();
    return () => {
      wsRef.current?.close();
  ...
  function handleAllClear() {
    Alert.alert(
      "Send All Clear?",
      "This will immediately stop all broadcasted alerts for your unit. Only use once vehicles have passed.",
      [
        { text: "Cancel", style: "cancel" },
        {
          text: "Yes, All Clear",
          ...
          onPress: () => {
            if (wsRef.current?.readyState === WebSocket.OPEN) {
              wsRef.current.send(JSON.stringify({ type: "ems_clear", unit_id: emsUnitId }));
            }
  ...
  function sendResume() {
    Alert.alert(
      "Resume Alert?",
      "This will re-broadcast the active alert to all vehicles in range.",
      [
        { text: "Cancel", style: "cancel" },
        {
          text: "Yes, Resume",
          onPress: () => {
            if (wsRef.current?.readyState === WebSocket.OPEN) {
              wsRef.current.send(JSON.stringify({ type: "ems_resume", unit_id: emsUnitId }));
            }
            setIsPaused(false);
            setPauseReason("");
          },
  ...
  // ── Loading state while credentials load ───────────────────────────────────
      ...
      <Modal visible={showPauseModal} transparent animationType="slide">
        <View style={styles.modalOverlay}>
          <View style={styles.modalBox}>
            ...
            <Text style={styles.pauseModalSub}>Civilians will see a "Stand By" screen instead of the active alert.</Text>
            {PAUSE_REASONS.map((r) => (
...
const styles = StyleSheet.create({
  ...
  pausedBanner:           { flexDirection: "row", alignItems: "center", gap: 12, backgroundColor: "#1c1500", borderRadius: 12, padding: 14, marginBottom: 12, borderWidth: 1, borderColor: "#d97706" },
